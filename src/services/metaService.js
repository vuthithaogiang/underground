...
export const connectMetaMaskService = async () => {
    try {
        ...
        const responseGetNonce = await axiosClient.get(`/auth/nonce?walletAddress=${accounts[0]}`);

        const { body: nonce } = responseGetNonce.data;

        const { publicAddress, signature } = await handleSignMessage(accounts[0], nonce);
